...
export default function AppNavbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      scrolled ? "bg-white/90 backdrop-blur-xl shadow-md py-3" : "bg-white py-5"
    }`}>
      <div className="container mx-auto px-6 flex items-center justify-between">
        ...
        <Link href={isVendor ? "/aliado" : "/catalog"}>
          <div className="flex items-center gap-3 cursor-pointer group">
            ...
            <div className="flex flex-col">
              ...
              {isVendor && <span className="text-[8px] font-bold text-green-600 tracking-widest">BUSINESS PARTNER</span>}
            </div>
          </div>
        </Link>
        ...
        <div className="hidden md:flex items-center bg-slate-100 p-1 rounded-full">
          <Link href={isVendor ? "/aliado" : "/catalog"}>
            <a className={`px-6 py-2 rounded-full text-xs font-bold transition-all ${location === (isVendor ? "/aliado" : "/catalog") ? "bg-white text-green-600 shadow-sm" : "text-slate-500"}`}>
              {isVendor ? "Panel" : "Explorar"}
            </a>
          </Link>
          <Link href={isVendor ? "/pedidos-recibir" : "/mis-rescates"}>
            <a className={`px-6 py-2 rounded-full text-xs font-bold transition-all ${location.includes("pedidos") || location.includes("rescates") ? "bg-white text-green-600 shadow-sm" : "text-slate-500"}`}>
        ...
        <div className="flex items-center gap-3">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" className="h-10 w-10 rounded-full border-2 border-white shadow-md ring-1 ring-slate-200">
            ...
            <DropdownMenuContent align="end" className="w-64 p-2 rounded-2xl">
              ...
              <Link href={isVendor ? "/perfil-aliado" : "/perfil"}>
                <DropdownMenuItem className="cursor-pointer rounded-xl font-bold py-3">
                  <User className="mr-2 h-4 w-4" /> Mi Cuenta
                </DropdownMenuItem>
              </Link>
              <DropdownMenuItem onClick={() => { localStorage.clear(); window.location.href="/"; }} className="text-red-600 font-bold rounded-xl py-3 cursor-pointer">
